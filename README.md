...
本專案批次測試共 30 題，針對 25 題可回答題與 5 題無答案題進行評估，Advanced RAG 的失敗案例集中於 **E3 (Reranking)** 錯誤，並潛在受到 **E1 (Chunking)** 的影響，分析如下：

### 1. E3：Reranking 重排篩選漏失 (主要錯誤來源 🌟)
* **現象描述**：Gold Chunk（標準答案片段）已成功進入第一階段 Top-20 候選，但在第二階段重排篩選時，被排除在 Top-5 之外。
* **評估結果**：此類錯誤發生了 5 次（在 25 題可回答題中，命中 20 題，漏失 5 題）。
* **原因分析**：
  在本次評估中，Advanced RAG 的向量檢索召回率 `candidate_recall_at_20` 為 **100%**，表示答案片段其實 100% 都有成功被召回。但重排命中率 `reranked_hit_at_5` 為 **80%**，這 20%（5 題）的落差即是答案片段在第二階段重排後，被排在了第 6 名或更後面。
  當正確答案未進入 Top-5 Context 中，Gemini 模型嚴格遵循防禦準則安全拒答（回覆 `文件中沒有足夠資訊`）。這證明模型行為 100% 遵守邊界規範，沒有任何捏造回答（E5 幻覺錯誤），此 5 題未答對純屬第二階段重排的篩選漏失（E3）。

### 2. E1：Chunking 固定長度切片造成語境切斷 (次要潛在錯誤 🧩)
* **現象描述**：規格書中連續的條款或表格在固定長度切片下被強行切開，導致語義不完整。
* **原因分析**：
  系統目前採用基於規則的固定長度切片（500 字元）。若規格書中的關鍵條款（例如：「前置條件」與其相對應的「例外限制」）跨度較長，或表格資料跨多行時，會被強制切碎在相鄰的 Chunks 中。這會導致重排模型（Reranker）在單獨比對時因局部資訊不完整而給出低分，進而引發後續的 E3 重排漏失。
