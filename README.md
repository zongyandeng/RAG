...
```text
project/
├── .env                   # 存放敏感金鑰（Hermes API Key & Telegram Token）
...
當你執行完 `evaluate.py` 後，系統會自動在 `results/summary.json` 計算各項指標：
*   **AC-01** (可執行性)：批次執行成功率 $\ge 95\%$。
*   **AC-02** (Candidate Recall@20)：可回答題的找回率 $\ge 80\%$。
*   **AC-03** (Reranked Hit@5)：重排後精選 Top-5 的找回率 $\ge 75\%$。
*   **AC-06** (Token 降低率)：Advanced RAG 相比 Long-context Baseline 的平均 LLM Input Tokens 降低 $\ge 50\%$。
*   **AC-08** (拒答率)：5 題無答案題中，正確拒答至少 3 題（$\ge 60\%$）。
*   **AC-10** (延遲量測)：詳細記錄 Embedding、Search、Rerank、總耗時與 TTFT 延遲。
